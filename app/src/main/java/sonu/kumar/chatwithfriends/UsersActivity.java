...
public class UsersActivity extends AppCompatActivity {
    ...
    private void doOnSuccess(String s) {
        try {
            ...
            Iterator i = obj.keys();
            Log.d(TAG, "doOnSuccess: iteroror keys"+i);
            String key = "";

            while(i.hasNext()){
                Log.d(TAG, "doOnSuccess: while loop"+i.hasNext());
                key = i.next().toString();
                Log.d(TAG, "doOnSuccess: while loop key is "+key);

                if(!key.equals(UserDetails.username)) {
                    Log.d(TAG, "doOnSuccess: key not equal to username");
                    al.add(key);
                }
